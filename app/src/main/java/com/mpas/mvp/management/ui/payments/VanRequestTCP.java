...
public class VanRequestTCP {
    public interface OnVanResponse {
        void onResponse (HashMap<String,String> responseMap);
    }
    private VanRequest.OnVanResponse onVanResponse;
    LinkedHashMap<String, byte[]> authMap;
    LinkedHashMap<String, byte[]> headerMap;
    VanRequestTCP.SocketThread thread;

    public void execute(PayUtil.AuthItem item, VanRequest.OnVanResponse vanResponse){
        ...
        try {
            ...
            thread = new VanRequestTCP.SocketThread(byteBuffer.array());
            thread.start();
